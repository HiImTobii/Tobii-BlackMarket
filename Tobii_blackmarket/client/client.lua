...
local function doContainerAnim(index, containerNetId, lockNetId, collisionNetId)
    ...
    TriggerServerEvent("Tobii_blackmarket_sv:openContainer", index)
...
RegisterNetEvent("Tobii_blackmarket_cl:openUI", function()
    displayUI(true)
end)

RegisterNetEvent("Tobii_blackmarket_cl:updateMarketItems", function(items)
    marketItems = items
...
RegisterNetEvent("Tobii_blackmarket_cl:updateStock", function(items, orderSrc)
    marketItems = items
...
RegisterNetEvent("Tobii_blackmarket_cl:orderReady", function(index, coords)
    ...
    TriggerServerEvent("Tobii_blackmarket_sv:propsSpawned", netIds, index)
...
RegisterNetEvent("Tobii_blackmarket_cl:addLockTarget", function(index, coords)
        ...
        options = {
            {
                ...
                label = "Open",
                action = function()
                    TriggerServerEvent("Tobii_blackmarket_sv:attemptContainer", index)
                end
            }
        }, 
        distance = 2.0
...
RegisterNetEvent("Tobii_blackmarket_cl:openContainer", function(index, propIds)
    doContainerAnim(index, propIds.container, propIds.lock, propIds.collision)
end)

RegisterNetEvent("Tobii_blackmarket_cl:updateOpenContainer", function(index, containerNetId, collisionNetId, crateCoords, removeLockTarget)
    local container = NetworkGetEntityFromNetworkId(containerNetId)
    local collision = NetworkGetEntityFromNetworkId(collisionNetId)
        ...
        options = {
            {
                ...
                label = "Loot",
                action = function()
                    TriggerServerEvent("Tobii_blackmarket_sv:attemptLoot", index)
                end
            }
        }, 
        distance = 2.0
...
RegisterNetEvent("Tobii_blackmarket_cl:removeLockTarget", function(index)
...
RegisterNetEvent("Tobii_blackmarket_cl:removeLootTarget", function(index)
...
RegisterNetEvent("Tobii_blackmarket_cl:lootContainer", function(index)
    local ped = PlayerPedId()
    ...
    QBCore.Functions.Progressbar("looting_crate", "Grabbing Items..", Config.lootTime, false, true, {
        ...
        TriggerServerEvent("Tobii_blackmarket_sv:finishLooting", index)
    end, function() -- Cancel
        TriggerServerEvent("Tobii_blackmarket_sv:cancelLooting", index)
        QBCore.Functions.Notify("Cancelled", "error")
    end)
end)

RegisterNetEvent("Tobii_blackmarket_cl:orderComplete", function()
    SendNUIMessage({
        action = "clearOrder"
    })
end)

RegisterNetEvent("Tobii_blackmarket_cl:hasPendingOrder", function(items, order, epochTime)
    SendNUIMessage({
...
RegisterNetEvent("Tobii_blackmarket_cl:enableLocateButton", function(index)
...
RegisterNUICallback("getClientData", function(data, cb)
    ...
    if not marketItems then
        QBCore.Functions.TriggerCallback("Tobii_blackmarket_sv:getMarketItems", function(items)
            marketItems = items
...
RegisterNUICallback("submitOrder", function(data, cb)
    QBCore.Functions.TriggerCallback("Tobii_blackmarket_sv:attemptOrder", function(result)
        cb(result)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    TriggerServerEvent("Tobii_blackmarket_sv:initPendingOrders")
end)
