...
local function getMarketItems()
    if not Config.randomItems then
        ...
        TriggerClientEvent("Tobii_blackmarket_cl:updateMarketItems", -1, marketItems)
...
local function orderComplete(index, stashId)
    ...
    if pendingOrders[index].isOpen then
        TriggerClientEvent("Tobii_blackmarket_cl:removeLootTarget", -1, index)
    else
        TriggerClientEvent("Tobii_blackmarket_cl:removeLockTarget", -1, index)
    end
    ...
    if pendingOrders[index].src then
        ...
        if Player.PlayerData.citizenid == pendingOrders[index].cid then
            TriggerClientEvent("Tobii_blackmarket_cl:orderComplete", pendingOrders[index].src)
        end
    end
...
local function orderReady(index, stashId)
    if pendingOrders[index].src then
        local Player = QBCore.Functions.GetPlayer(pendingOrders[index].src)
        if Player.PlayerData.citizenid == pendingOrders[index].cid then
            TriggerClientEvent("Tobii_blackmarket_cl:orderReady", pendingOrders[index].src, index, Config.deliveryLocations[index])
        end
    end
...
QBCore.Functions.CreateUseableItem(Config.useItem, function(source)
    ...
    TriggerClientEvent("Tobii_blackmarket_cl:openUI", source)
end)

QBCore.Functions.CreateCallback("Tobii_blackmarket_sv:getMarketItems", function(src, cb)
    cb(marketItems)
end)

QBCore.Functions.CreateCallback("Tobii_blackmarket_sv:attemptOrder", function(src, cb, order)
    ...
    if Player.Functions.RemoveMoney(Config.paymentType, cost) then
        ...
        TriggerClientEvent("Tobii_blackmarket_cl:updateStock", -1, marketItems, src)
...
RegisterNetEvent("Tobii_blackmarket_sv:propsSpawned", function(netIds, locationIndex)
    ...
    TriggerClientEvent("Tobii_blackmarket_cl:addLockTarget", -1, locationIndex, lockCoords)
end)

RegisterNetEvent("Tobii_blackmarket_sv:attemptContainer", function(index)
    local src = source
    ...
    TriggerClientEvent("Tobii_blackmarket_cl:openContainer", src, index, pendingOrders[index].props)
end)

RegisterNetEvent("Tobii_blackmarket_sv:openContainer", function(index)
    local src = source
    ...
    TriggerClientEvent("Tobii_blackmarket_cl:updateOpenContainer", -1, index, pendingOrders[index].props.container, pendingOrders[index].props.collision, crateCoords, true)
end)

RegisterNetEvent("Tobii_blackmarket_sv:attemptLoot", function(index)
    local src = source
    if #(GetEntityCoords(GetPlayerPed(src)) - vec3(Config.deliveryLocations[index])) > 5 or not pendingOrders[index] then return end
    ...
    TriggerClientEvent("Tobii_blackmarket_cl:lootContainer", src, index)
end)

RegisterNetEvent("Tobii_blackmarket_sv:finishLooting", function(index)
    local src = source
...
RegisterNetEvent("Tobii_blackmarket_sv:cancelLooting", function(index)
    pendingOrders[index].lootInProgress = false
end)

RegisterNetEvent("Tobii_blackmarket_sv:initPendingOrders", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    for k, v in pairs(pendingOrders) do
        ...
        if v.cid == Player.PlayerData.citizenid then
            v.src = src
            TriggerClientEvent("Tobii_blackmarket_cl:hasPendingOrder", src, marketItems, v.order, v.deliveryTime)      
            ...
            if os.time() - v.deliveryTime > 0 then
                if not v.props then
                    ...
                    if v.isOpen then 
                        TriggerClientEvent("Tobii_blackmarket_cl:removeLootTarget", -1, k)
                        v.isOpen = false
                    end
                    TriggerClientEvent("Tobii_blackmarket_cl:orderReady", src, k, Config.deliveryLocations[k])
                else
                    TriggerClientEvent("Tobii_blackmarket_cl:enableLocateButton", src, k)
                end
            end
        end
        ...
        if v.props then
            if not v.isOpen then
                ...
                local lockCoords = vec4(GetEntityCoords(lock), GetEntityHeading(lock))
                TriggerClientEvent("Tobii_blackmarket_cl:addLockTarget", src, k, lockCoords)
            else
                local crate = NetworkGetEntityFromNetworkId(v.props.crate)
                local crateCoords = vec4(GetEntityCoords(crate), GetEntityHeading(crate))

                TriggerClientEvent("Tobii_blackmarket_cl:updateOpenContainer", src, k, v.props.container, v.props.collision, crateCoords, false)
            end
        end
        
    end
    
    TriggerClientEvent("Tobii_blackmarket_cl:updateMarketItems", src, marketItems)
end)
